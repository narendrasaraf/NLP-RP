...
class NLPAnalyzer:
    """
    Lightweight rule-based NLP module for real-time game chat analysis.
    Extracts polarity, intensity, and probabilities of specific emotional states.
    """
    def __init__(self):
        # Lexicons containing game-specific jargon
        ...
        self.strategic_words = {'cover', 'go', 'push', 'defend', 'attack', 'help', 'left', 'right', 'back', 'up', 'down', 'heal', 'drop', 'need', 'flank', 'rush', 'hold', 'plant', 'defuse', 'guard'}
        
        # Maintain history of last 5 localized contexts
        self.message_history = []

    def process_chat(self, text: str) -> dict:
        """
        Takes a chat message string and returns exactly 5 requested metrics.
        """
        ...
        for word in words:
            ...
            if word in self.strategic_words:
                score_strat += val if not is_negated else 0
        ...
        uppercase_ratio = caps_count / max(len(raw_words), 1)
        negative_word_ratio = negative_words_count / max(total_words, 1)
        
        # Calculate sentiment shift relative to last message
        sentiment_shift = 0.0
        if self.message_history:
            last_polarity = self.message_history[-1].get("polarity", 0.0)
            sentiment_shift = float(round(raw_polarity - last_polarity, 3))
        ...
        intent_scores = {
            "strategic": score_strat,
            "frustration": score_frust,
        ...
        if intent_scores[max_intent] > 0:
            ...
            intent_confidence = min(1.0, intent_scores[max_intent] / math.ceil(total_words/3))
            
            # Simple heuristic goal extraction
            if intent_type == "strategic":
                # Find the first strategic word and grab a small window of context
                for i, w in enumerate(words):
                    if w in self.strategic_words:
                        start = max(0, i - 1)
                        end = min(len(words), i + 3)
                        goal_expression = " ".join(words[start:end])
                        break
            else:
                # Capture the core expression up to a max length for frustration/confidence
                goal_expression = text.strip()[:50]
        ...
        result = {
            ...
            "uppercase_ratio": float(round(uppercase_ratio, 3)),
            "negative_word_ratio": float(round(negative_word_ratio, 3)),
            "sentiment_shift": sentiment_shift,
            
            # Intent Field
            "intent_type": intent_type,
            "intent_confidence": float(round(intent_confidence, 3)),
            "goal_expression": goal_expression
        }
        
        # Maintain rolling window of 5 message histories
